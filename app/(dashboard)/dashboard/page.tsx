        ...
        <div className="bg-background border border-border rounded-2xl overflow-hidden shadow-xl">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-card border-b border-border text-muted-foreground text-base font-semibold">
                <tr>
                  <th className="px-4 md:px-6 py-4 font-semibold">Product Name</th>
                  <th className="px-4 md:px-6 py-4 font-semibold hidden sm:table-cell">Status</th>
                  <th className="px-4 md:px-6 py-4 font-semibold text-right sm:text-left">Stock</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {data?.productSummary?.map((product: any) => (
                  <tr key={product._id} className="hover:bg-card transition-colors">
                    <td className="px-4 md:px-6 py-4 text-card-foreground font-semibold text-base">{product.name}</td>
                    <td className="px-4 md:px-6 py-4 hidden sm:table-cell">
                      <span className={`px-3.5 py-1 text-sm font-semibold rounded-full border ${
                    ...
                    <td className="px-4 md:px-6 py-4 text-card-foreground font-semibold text-base text-right sm:text-left">{product.stock} units</td>
                  </tr>
                ))}
